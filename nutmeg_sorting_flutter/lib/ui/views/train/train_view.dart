  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<TrainViewModel>.reactive(
      ...
      builder: (context, model, child) {
        ...
        return Scaffold(
            appBar: AppBar(
              title: const Text('Ring Training'),
              centerTitle: true,
  ...
  Widget build(BuildContext context, TrainViewModel model) {
    ...
    return Stack(
      children: [
                        ...
                        children: [
                          _OtherButtons(
                            ...
                            onTap: model.isSorting
                                ? model.stopSorting
                                : model.sort,
                            isLoading: !model.isSorting && model.isRingPlaced,
                          ),
                          if (!model.isSorting &&
                              !model.isTakingDecision &&
                              !model.isRingPlaced)
                            _NormalButton(
        ...
        if (model.nutImg != null)
          Center(child: ImageViewer(img: model.nutImg!)),
        // if (model.bottomImg != null)
        //   Positioned(
        //     bottom: MediaQuery.of(context).size.height * 0.3 + 10,
        //     child: ImageViewer(img: model.bottomImg!, isTop: false),
        //   ),
        if (model.isTakingDecision)
          Positioned(
            top: 0,
            left: 0,
            right: 0,
...
class _NormalButtonTruth extends ViewModelWidget<TrainViewModel> {
  final String text;
  final IconData icon;
  final VoidCallback onTap;
  final bool isTrue;
  final Color color;
  const _NormalButtonTruth({
    required this.text,
    required this.icon,
    required this.onTap,
    required this.isTrue,
    ...
    Widget _buildThermometer(BuildContext context) {
      return Padding(
        ...
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(20),
            color: isTrue ? color : color.withOpacity(0.5),
            boxShadow: isTrue
                ? [
                    BoxShadow(
                      color: Colors.grey,
